...
const Footer: React.FC = () => {
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      const newSubscribers = [...data.subscribers, { email, date: new Date().toISOString() }];
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-3 text-slate-400">
              {data.services.slice(0, 4).map((s: any) => (
                <li key={s.id} className="hover:text-emerald-500 cursor-pointer">{t('', s.title)}</li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-slate-400">
              ...
              <li className="flex items-center gap-2">
                <svg className="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 01-2 2H7a2 2 0 01-2-2zm0-11V7a2 2 0 012-2h10a2 2 0 012 2v10a2 2 0 01-2 2H7a2 2 0 01-2-2V8z"/></svg>
          ...
          <div>
            <h4 className="font-bold text-lg mb-6">{lang === 'en' ? 'Newsletter' : 'النشرة البريدية'}</h4>
            <form onSubmit={handleSubscribe} className="space-y-4">
              <input
                ...
                required
              />
              <button className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 rounded-lg transition-colors">
                {lang === 'en' ? 'Subscribe' : 'اشترك'}
              </button>
              {subscribed && (
                <p className="text-emerald-400 text-sm animate-pulse">
                  {lang === 'en' ? 'Subscribed successfully!' : 'تم الاشتراك بنجاح!'}
                </p>
              )}
            </form>
          </div>
